...
class AScore:
    ...
    def updateMembers_(self):
        ...
        # No need to update members as we're not using DefaultParamHandler anymore
        pass
    ...
    def compute(self, hit, real_spectrum):
        ...
        # reset phospho
        phospho.setScore(0.0)  # Initialize to 0.0 instead of -1
        ...
        if number_of_sites == number_of_phosphorylation_events:
            # Only use unambiguous_score_ when all possible sites are phosphorylated
            ...
            # Set AScore for each phosphorylation site
        ...
        # Check if there are enough permutations for comparison
        ...
        rank = 1
        best_Ascore = 0.0  # Changed from float('inf') to 0.0
        site2score = {}
        ...
        for phospho_site in phospho_sites:
            ...
            if peptide1_score == peptide2_score:  # Set Ascore = 0 for each phosphorylation site
                ...
                Ascore = 0.0  # Explicitly set to 0.0
            else:
                ...
                # Check if site_determining_ions is empty
                if not site_determining_ions or len(site_determining_ions) < 2 or site_determining_ions[0].size() == 0 or site_determining_ions[1].size() == 0:
                    print(f"Debug: No site-determining ions found for phosphorylation site {phospho_site['first']}")
                    Ascore = 0.0
                else:
            ...
            if Ascore > best_Ascore:  # Changed from < to > since higher scores are better
                best_Ascore = Ascore
    ...
    def determineHighestScoringPermutations_(self, peptide_site_scores, sites, permutations, ranking):
        ...
        for i in range(len(best_peptide_sites)):  # For each phosphorylated site
            ...
            if rank_index < len(ranking):
                ...
                # Ensure peptide_site_scores has enough depth data
    ...
    def calculatePermutationPeptideScores_(self, th_spectra, windows_top10):
        ...
        for idx, spectrum in enumerate(th_spectra):
            ...
            for i in range(1, 11):  # Peak depths 1-10
                ...
                # Ensure cumulative_score is not less than or equal to 0
                if cumulative_score <= 0:
                    # Use a very small positive number instead of 0 or negative
                    cumulative_score = 1e-100
