...
class PSM:
    ...
    def __init__(self, peptide: Peptide, spectrum_source: Union[str, Dict, Spectrum], scan_num: Optional[int] = None, config: Dict = None):
        """
        Initialize PSM.
        ...
        if isinstance(spectrum_source, str):
            ...
            self.spectrum = None
            if scan_num is None:
                raise ValueError("scan_num is required when spectrum_source is a file path")
            self._load_spectrum()
    ...
    def _init_modifications(self):
        ...
        try:
            ...
            if seq.startswith("["):
                ...
                self.mod_pos_map[NTERM_MOD] = 0.0  # Add N-terminal modification position
                seq = seq[1:]
            
            # Process C-terminal modifications
            if seq.endswith("]"):
                self.mod_coord_map[CTERM_MOD] = 0.0
                self.mod_pos_map[CTERM_MOD] = len(seq)  # Add C-terminal modification position
                seq = seq[:-1]
    ...
    def _extract_scan_number(self, spectrum_data: Dict) -> int:
        ...
        try:
            ...
            if 'scan=' in spectrum_id:
                ...
                logger.warning(f"Could not extract scan number from spectrum ID: {spectrum_id}")
                return 0
        except Exception as e:
    ...
    def process(self, config: Optional[Dict] = None, round_number: int = 0) -> None:
        ...
        if hasattr(self, 'flr_calculator') and self.flr_calculator is not None and round_number == 0:
            ...
            if self.delta_score > self.flr_calculator.min_delta_score:
                ...
                logger.debug(f"Added PSM to FLR calculator - delta_score: {self.delta_score:.6f}, is_decoy: {self.is_decoy}")
            else:
                logger.debug(f"PSM delta_score {self.delta_score:.6f} <= {self.flr_calculator.min_delta_score}, skipping FLR calculation")
        ...
        if self.delta_score == 0.0 and self.psm_score == 0.0:
            ...
            if real_scores:
                ...
                self.delta_score = sorted_real[0] - sorted_real[1] if len(sorted_real) > 1 else sorted_real[0]
            else:
                self.psm_score = 0.0
                self.delta_score = 0.0
    ...
    def process_round2(self, flr_calculator) -> None:
        ...
        if flr_calculator and hasattr(flr_calculator, 'find_closest_flr'):
            if self.delta_score > flr_calculator.min_delta_score:
                global_flr, local_flr = flr_calculator.find_closest_flr(self.delta_score)
        ...
        logger.debug(f"Second round processing completed - Delta Score: {self.delta_score:.6f}, Global FLR: {self.global_flr:.6f}, Local FLR: {self.local_flr:.6f}")
    ...
    def generate_permutations_stage2(self) -> None:
        ...
        logger.debug(f"Second stage generated {len(real_permutations)} real permutations")
        
        # Store real permutations (don't generate decoy permutations)
        for perm, sites in real_permutations:
            self.pos_permutation_score_map[perm] = 0.0  # Score will be calculated in scoring stage
    ...
    def _validate_permutation(self, perm: str) -> bool:
        ...
        try:
            ...
            if perm_unmod_len != orig_unmod_len:
                logger.debug(f"Unmodified sequence length mismatch: {perm_unmod_len} != {orig_unmod_len}")
                return False
    ...
    def score_permutations(self, model: Optional[object]) -> None:
        ...
        try:
            ...
            all_perms = list(self.pos_permutation_score_map.keys()) + list(self.neg_permutation_score_map.keys())
            base_tolerance = self.config.get('fragment_mass_tolerance', 0.1) # Read from config, default 0.1
            ...
            for perm in all_perms:
                ...
                for peak in matched_peaks:
                    ...
                    if ion_type.startswith('b'):
                        intensity_m = model.get_log_np_density_int('b', peak['norm_intensity'], self.charge)
                    elif ion_type.startswith('y'):
                        intensity_m = model.get_log_np_density_int('y', peak['norm_intensity'], self.charge)
                    
                    intensity_u = model.get_log_np_density_int('n', peak['norm_intensity'], self.charge)
                    ...
                    if ion_type.startswith('b'):
                        dist_m = model.get_log_np_density_dist_pos(peak['mass_diff'], self.charge)
                    elif ion_type.startswith('y'):
                        dist_m = model.get_log_np_density_dist_pos(peak['mass_diff'], self.charge)
                ...
                logger.debug(f"Scoring permutation: {perm}, Score: {final_score:.6f}, Is decoy: {is_decoy}")
            ...
            score2 = 0.0
            if not self.is_unambiguous:
                score2 = self._round_dbl(all_scores[1], 6) if len(all_scores) > 1 else 0.0
            ...
            if self.is_unambiguous:
                ...
                logger.debug(f"Non-unambiguous case, delta score: {score1} - {score2} = {self.delta_score}")
    ...
    def _calc_theoretical_masses(self, perm: str) -> List[float]:
        ...
        for i in range(len(perm)):
            ...
            if i >= 1:  # Minimum length is 2
                ...
                for z in range(1, self.charge):  # Only generate charge states 1 to charge-1
    ...
    def generate_real_permutations(self) -> List[Tuple[str, List[int]]]:
        ...
        try:
            ...
            num_rps = len([m for m in self.mod_coord_map.values() if abs(m - PHOSPHO_MOD_MASS) < 0.01])
    ...
    def generate_decoy_permutations(self) -> List[Tuple[str, List[int]]]:
        ...
        try:
            ...
            num_mods = len([m for m in self.mod_coord_map.values() if abs(m - PHOSPHO_MOD_MASS) < 0.01])
            if num_mods == 0:
                logger.debug("No phosphorylation modifications found in peptide")
                return []
            ...
            for sites in islice(combos, max_decoys):
                ...
                for i, aa in enumerate(peptide_seq):
                    if i in sites:  # Sites that need modification
                        ...
                        mod_pep += aa.upper()  # Non-modification sites use uppercase letters
    ...
    def generate_permutations(self, run_number: int) -> None:
        ...
        """
        logger.debug(f"Generating permutations for PSM {self.scan_num}, run number: {run_number}")
        ...
        if not self.is_unambiguous and run_number == 0:
            ...
            logger.debug(f"PSM {self.scan_num} generated {len(self.pos_permutation_score_map)} real permutations and {len(self.neg_permutation_score_map)} decoy permutations")
        else:
            # Second iteration or unambiguous PSM: only generate real permutations
            logger.debug(f"PSM {self.scan_num} generated {len(self.pos_permutation_score_map)} real permutations (no decoy)")
    ...
    def update_peptide_id(self) -> None:
        ...
        if not hasattr(self, 'peptide_hit') or not hasattr(self, 'peptide_id'):
            logger.warning("PSM does not have original peptide_hit or peptide_id data, cannot update")
            return
        ...
        try:
            ...
            logger.debug(f"Updated peptide identification data for PSM {self.scan_num}, delta_score: {self.delta_score:.6f}, global_flr: {self.global_flr:.6f}, local_flr: {self.local_flr:.6f}")
    ...
    def calculate_delta_score(self, include_decoys: bool = True) -> None:
        ...
        try:
            ...
            if len(real_scores) == 0:
                logger.debug(f"PSM {self.scan_num} has no real permutation scores, setting delta_score to 0")
                self.delta_score = 0.0
                return
            ...
            if include_decoys and len(self.neg_permutation_score_map) > 0:
                ...
                if len(real_scores) == 1:
                    ...
                    self.delta_score = top_score - second_score
                    logger.debug(f"PSM {self.scan_num} second round delta_score (real permutations only): {top_score:.6f} - {second_score:.6f} = {self.delta_score:.6f}")
            
        except Exception as e:
            logger.error(f"Error calculating delta_score for PSM {self.scan_num}: {str(e)}")
            self.delta_score = 0.0
