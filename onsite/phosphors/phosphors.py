...
DEFAULT_OCCURRENCE_PROBABILITY = 0.1  # Default occurrence probability if calculation fails
...
def _add_distribution_to_cache(p: float, n: int, prob: float):
    ...
    if len(_distribution_cache) >= DISTRIBUTION_CACHE_SIZE:
        ...
        keys_to_remove = list(_distribution_cache.keys())[:len(_distribution_cache) - DISTRIBUTION_CACHE_SIZE + 1]
...
def binomial_tail_probability(k: int, n: int, p: float) -> float:
    ...
    if k > expected + 2 * std_dev:
        ...
        for t in range(k, n + 1):
            ...
            log_1_minus_p_term = (n - t) * math.log(1.0 - p) if (1.0 - p) > 0 else float('-inf')
        ...
        log_prob = max_log_term + math.log(sum(math.exp(log_term - max_log_term) for log_term in log_terms))
...
def _count_peaks_above_threshold(intensity: list, start: int, end: int, thr: float) -> int:
    c = 0
...
def _reduce_spectrum_by_windows(exp_spectrum: MSSpectrum, tol_da_for_p: float, is_ppm: bool, fragment_tolerance_value: float) -> tuple:
    """
        ...
        if end - start > 0:
            ...
            if thresholds:
                ...
                for depth_idx in range(1, len(thresholds) + 1):
                    ...
                    bigp = binomial_tail_probability(max(1, min(n_peaks, 1)), n_peaks if n_peaks > 0 else 1, p_win)
                    if bigp < best_bigp:
                        best_bigp = bigp
                        best_i = depth_idx - 1
...
def filter_spectrum_for_phosphors(exp_spectrum: MSSpectrum, fragment_tolerance: float, fragment_method_ppm: bool) -> MSSpectrum:
    """
    Filter spectrum:
...
def _generate_isomer_profiles(original_sequence: AASequence, potential_site_indices: list, num_mods_present: int, modification_name: str, add_decoys: bool = False) -> list:
...
def _expected_fragment_mzs(seq: AASequence, precursor_charge: int, add_neutral_losses: bool) -> list:
    ...
    for ion_type in ["a", "b", "c", "x", "y", "z"]:
        params.setValue(f"add_{ion_type}_ions", "true" if ion_type in ("b", "y") else "false")
    spec_gen.setParameters(params)
    theo = MSSpectrum()
    try:
        # Limit fragment charge to (precursor_charge - 1) and not exceeding MAX_ION_CHARGE
        max_frag_z = max(1, min(MAX_ION_CHARGE, (precursor_charge - 1) if precursor_charge and precursor_charge > 1 else 1))
...
def _site_determining_ions(profiles: list, precursor_charge: int, add_neutral_losses: bool) -> tuple:
...
def _window_intensity_thresholds(filtered_spec: MSSpectrum, start: int, end: int) -> list:
...
def _reduce_by_delta_selection(filtered_spec: MSSpectrum, profiles: list, fragment_tolerance: float, fragment_method_ppm: bool) -> MSSpectrum:
    ...
    sdi_map, profile_to_mzs, union_mzs = _site_determining_ions(profiles, precursor_charge, ADD_NEUTRAL_LOSSES)
    reduced_idx = []
    cur_min = min_mz
        ...
        if w_end - w_start > 0:
            ...
            if thresholds:
                ...
                for depth_idx in range(1, len(thresholds) + 1):
                    ...
                    if profile_to_sdi_mz:
                        ...
                        for prof_idx, (_seq, _sites) in enumerate(profiles):
                            ...
                            profile_theo_mzs = profile_to_mzs[prof_idx] if prof_idx < len(profile_to_mzs) else set()
                ...
                if all(len(d)==0 for d in deltas):
                                ...
                                for i_idx in range(w_start, w_end):
                                    if it_arr[i_idx] >= thr:
                                        ...
                                        if (mz_exp >= min_b) and (mz_exp < max_b):
                                            k += 1
                                            break
                        big_p = binomial_tail_probability(k, n_peaks if n_peaks > 0 else 1, p_win)
                        if big_p < best_bigp:
                            best_bigp = big_p
                            best_i = depth_idx - 1
                    ...
                    best_thr = thresholds[best_i]
                    window_best = [i for i in range(w_start, w_end) if it_arr[i] >= best_thr]
                    if window_best:
                    ...
                    for j in range(n_deltas):
                        if largest_delta == 0.0:
                            for i in range(len(deltas)):
                                ...
                                if j < len(temp_deltas) and temp_deltas[j] > largest_delta:
...
def calculate_phosphors_score(
    ...
    try:
        ...
        def get_intensity_weight(intensity):
            return math.log10(1 + intensity / total_intensity) if total_intensity > 0 else 1.0
...
def calculate_phospho_localization_compomics_style(
    ...
    try:
        ...
        if not valid_mods:
            raise ValueError(f"No valid phosphorylation modifications found for sites: {dynamic_potential_sites}")
        ...
        try:
            ...
            if add_decoys:
                ...
                num_mods_present = (str(original_sequence).count(f"({MODIFICATION_NAME})") + 
            ...
            if num_mods_present == 0:
                ...
                if abs(mod_mass) > 1e-6:
                    ...
                    print(f"Warning: Modification mass is zero. Using string count: {num_mods_present}")
        ...
        if len(potential_site_indices) == num_mods_present:
            ...
            trivial_probs = {idx: 100.0 for idx in potential_site_indices}  # Already percentage
        ...
        filtered_spec = filter_spectrum_for_phosphors(spectrum, fragment_tolerance, fragment_method_ppm)
        ...
        phospho_rs_spec = _reduce_by_delta_selection(filtered_spec, profiles, fragment_tolerance, fragment_method_ppm)
        ...
        for seq_profile, site_indices_set in profiles:
            ...
            for mz_theo in theo_mz:
                tol_da = (mz_theo * fragment_tolerance / 1_000_000.0) if fragment_method_ppm else fragment_tolerance
            ...
            big_p = binomial_tail_probability(k_matches, n_expected if n_expected > 0 else 1, p_calc)
        ...
        for item in isomer_scores:
            ...
            for site_idx in item["sites"]:
                site_probabilities[site_idx] = site_probabilities.get(site_idx, 0.0) + prob
        ...
        isomer_list_out = [(str(item["isomer_seq"]), item["big_p"]) for item in isomer_scores]
...
if __name__ == "__main__":
    ...
    # Create peak list
    mz_array = [147.11, 276.15, 363.18, 499.13, 591.30, 772.31, 990.33, 300.1, 550.2, 800.3]
    intensity_array = [1000.0, 800.0, 600.0, 1800.0, 1400.0, 2000.0, 900.0, 200.0, 300.0, 240.0]
