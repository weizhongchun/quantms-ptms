...
def ascore(
    ...
    try:
        ...
        if threads == 1:
            ...
            for i, pid in enumerate(peptide_ids):
                if debug and i % 100 == 0:
                    logger.info(
                        f"Processing peptide identification {i+1}/{len(peptide_ids)}"
                    )
                process_peptide_identification(pid, exp, logger, add_decoys)
        else:
            # Parallel processing
            click.echo(
                f"[{time.strftime('%H:%M:%S')}] Processing {len(peptide_ids)} peptide identifications using {threads} threads..."
            )
                ...
                for future in as_completed(future_to_pid):
                    try:
                        ...
                        if debug and completed % 100 == 0:
                            logger.info(
                                f"Completed {completed}/{len(peptide_ids)} peptide identifications"
                            )
                    except Exception as exc:
                        pid = future_to_pid[future]
                        click.echo(
                            f"Peptide identification generated an exception: {exc}"
                        )
                        if debug:
                            logger.error(
                                f"Error processing peptide identification: {exc}"
                            )
        ...
        click.echo(
            f"[{time.strftime('%H:%M:%S')}] Processing completed in {processing_time:.2f} seconds"
        )
...
def save_identifications(out_file, protein_ids, peptide_ids):
    ...
    print(
        f"[{time.strftime('%H:%M:%S')}] Saving {len(peptide_ids)} peptide identifications to {out_file}"
    )
...
def _worker_process_pid(task):
    ...
    pid, mzml_file, fragment_mass_tolerance, fragment_mass_unit, add_decoys, debug = (
        task
    )
    ...
    try:
        ...
        for hit in pid.getHits():
            try:
                ...
                if not has_phospho:
                    logger.debug(
                        f"No phosphorylation sites found in {original_seq_str}"
                    )
                    hit.setMetaValue("AScore_pep_score", -1.0)
                    continue
                ...
                if new_seq_str != original_seq_str:
                    hit.setSequence(new_sequence)
                    logger.debug(
                        f"Sequence updated: {original_seq_str} -> {new_seq_str}"
                    )
