...
class AScore:
    ...
    def compute(self, hit, real_spectrum):
        ...
        if (not self.add_decoys_) and decoy_phospho_count > 0:
            self._get_logger().info("Warning: PhosphoDecoy sites found but add_decoys is false. Returning original hit.")
            return phospho
        ...
        phospho_sites = []
        self.determineHighestScoringPermutations_(peptide_site_scores, phospho_sites, permutations, ranking)
        ...
        for phospho_site in phospho_sites:
            ...
            if peptide1_score == peptide2_score:
                self._get_logger().info(f"Debug: Best and second best peptide scores are equal ({peptide1_score})")
                Ascore = 0.0
            else:
                site_determining_ions = []
                self.computeSiteDeterminingIons_(th_spectra, phospho_site, site_determining_ions)
                
                if not site_determining_ions or len(site_determining_ions) < 2 or site_determining_ions[0].size() == 0 or site_determining_ions[1].size() == 0:
                    self._get_logger().info(f"Debug: No site-determining ions found for site {phospho_site['first']}")
                    Ascore = 0.0
                else:
    ...
    def determineHighestScoringPermutations_(self, peptide_site_scores, sites, permutations, ranking):
        ...
        for i in range(len(best_peptide_sites)):
            ...
            if rank_index >= 0:
                ...
                max_depth = min(len(peptide_site_scores[site_info['seq_1']]), 
    ...
    def numberOfPhosphoEvents_(self, sequence):
        ...
        self._get_logger().info(f"Debug: Found {cnt_phospho_events} phosphorylation events in sequence: {sequence}")
    ...
    def computePermutations_(self, sites, n_phosphorylation_events):
        ...
        if self.max_permutations_ > 0 and n_phosphorylation_events >= 1:
            if n_phosphorylation_events > len(sites):
                self._get_logger().info(f"Debug: More phosphorylation events ({n_phosphorylation_events}) than available sites ({len(sites)})")
                return permutations
            else:
                try:
                    from math import comb
                ...
                if estimated_permutations > self.max_permutations_:
                    self._get_logger().info(f"Debug: Estimated permutations ({estimated_permutations}) exceeds maximum ({self.max_permutations_})")
                    return permutations
            ...
            try:
                ...
                for combo in combinations(sites, n_phosphorylation_events):
                    ...
                    if self.max_permutations_ > 0 and len(permutations) > self.max_permutations_:
                        self._get_logger().info(f"Debug: Early termination during iteration: current permutations ({len(permutations)}) exceeds maximum ({self.max_permutations_})")
                        return []
                return permutations
            except ImportError:
            ...
            for as_pos in range(seq.size()):
                if permu < len(permutations[i]) and as_pos == permutations[i][permu]:
                    ...
                    if residue in ['S', 'T', 'Y']:
                        ...
                        self._get_logger().info(f"Debug: Set Phospho modification at position {as_pos} (residue {residue})")
                    elif self.add_decoys_ and residue == 'A':
                        seq.setModification(as_pos, "PhosphoDecoy")
                        self._get_logger().info(f"Debug: Set PhosphoDecoy modification at position {as_pos} (residue {residue})")
